...
async function main() {
  ...
  console.log(`\nDeploying CipherHoldem to ${network}...`);

  const [deployer] = await hre.ethers.getSigners();
  console.log(`Deployer: ${deployer.address}`);
  console.log(`Balance:  ${hre.ethers.formatEther(await hre.ethers.provider.getBalance(deployer.address))} ETH\n`);

  const CipherHoldem = await hre.ethers.getContractFactory("CipherHoldem");
  const holdem = await CipherHoldem.deploy();
  await holdem.waitForDeployment();

  const address = await holdem.getAddress();
  console.log(`✅  CipherHoldem deployed at: ${address}`);
