...
type ToastType = 'success' | 'error' | 'info' | 'warning' | 'tx' | 'pending';
...
export const toast = {
  ...
  pending: (title: string, message?: string) => {
    const id = ++_nextId;
    useToastStore.getState().add({ type: 'pending', title, message, duration: 999_999 });
    return { dismiss: () => useToastStore.getState().remove(id) };
  },
};

const TYPE_CONFIG: Record<ToastType, { icon: string; color: string; bg: string; border: string; spin?: boolean }> = {
  success: { icon: '✓', color: '#00E86C', bg: 'rgba(0,232,108,0.08)', border: 'rgba(0,232,108,0.2)' },
  error:   { icon: '✕', color: '#FF3B3B', bg: 'rgba(255,59,59,0.08)', border: 'rgba(255,59,59,0.2)' },
  info:    { icon: 'i', color: '#5B9EFF', bg: 'rgba(91,158,255,0.08)', border: 'rgba(91,158,255,0.2)' },
  warning: { icon: '!', color: '#FFE03D', bg: 'rgba(255,224,61,0.08)', border: 'rgba(255,224,61,0.2)' },
  tx:      { icon: '⛓', color: '#B366FF', bg: 'rgba(179,102,255,0.08)', border: 'rgba(179,102,255,0.2)' },
  pending: { icon: '◌', color: '#00BFFF', bg: 'rgba(0,191,255,0.08)', border: 'rgba(0,191,255,0.2)', spin: true },
};
...
const ToastItem = ({ item, onDismiss }: { item: ToastItem; onDismiss: () => void }) => {
  ...
  return (
    ...
    >
      <div
        className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 font-mono text-xs font-bold mt-0.5 ${cfg.spin ? 'animate-spin' : ''}`}
