...
## 8. Статус реалізації Wave 5 (оновлено 2026-05-22)

**Блоки A + B — РЕАЛІЗОВАНО, ЗАДЕПЛОЄНО і ПЕРЕВІРЕНО on-chain.**

Що зроблено:
- `balances` у обох PvP-контрактах → зашифрований `euint64` **банкрол**. On-chain ніхто
  (опонент, валідатор, спектатор) не може його прочитати — лише власник через permit.
  `getBalance()` повертає ciphertext-хендл.
- Buy-in дебетує банкрол гомоморфно (`FHE.sub` із `FHE.min`-clamp проти underflow) і
  відкриває плейнтекст-стек столу. Betting-рушій працює на плейнтекст-стеку — незмінний.
- Архітектурне уточнення під час реалізації: повністю зашифрувати *живий* стек за столом
  із чистим FHE неможливо (ігровий цикл мусить знати суми) — тому зашифровано **банкрол
  за столом** (модель Tongo): твоє загальне багатство приватне, на стіл виставляєш публічний buy-in.
- Buy-in зроблено **синхронним**: початкова версія з `createDecryptTask` на `euint64`
  ревёртила on-chain (CoFHE `createDecryptTask` працює лише з `ebool`).

Задеплоєно на Ethereum Sepolia:
- `CofhePokerPvP`  — `0x7f29231Dfb9Ea271B3C39A494D3274f311952923`
- `CofheHoldemPvP` — `0xc3167bBbcC9a3Abff3420D622E992F7C95C45fB5`

Доказ (on-chain smoke-тест `scripts/verify-funding.mjs`, Sepolia):
```
createPvPTable(buyIn=100) → status=1 ✓
getStackOf  → 100                  (стек = buy-in)        PASS
getBalance  → 426947284539845347…  (ciphertext-хендл)     PASS
leaveTable  → cash-out у банкрол                          ✓
```

Залишок: Блок D (traction — прогін `bot.mjs` для on-chain активності), повний UI-тест
PvP-хенду, перемикання Vercel env на нові адреси після ручної перевірки.
